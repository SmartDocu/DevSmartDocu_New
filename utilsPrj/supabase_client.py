...
def get_thread_supabase(access_token=None, refresh_token=None):
    ...
    # access_token이 있으면 postgrest 요청 헤더의 Authorization을 user JWT로 덮어씀
    # (pg.headers가 session.headers보다 우선 적용되므로 직접 수정 필요)
    if access_token:
        try:
            _thread_local.client.postgrest.headers["Authorization"] = f"Bearer {access_token}"
        except Exception:
            pass
        try:
            # Storage 클라이언트도 user JWT 사용하도록 session.headers 직접 업데이트
            # storage3 v0.11+: client.storage.session.headers 가 실제 HTTP 요청 헤더
            _thread_local.client.storage.session.headers.update({"Authorization": f"Bearer {access_token}"})
        except Exception:
            pass
...
# 클라이언트 생성 — 비잠금 평범한 클라이언트 (병렬 워커 스레드용)
# get_thread_supabase의 LockedClient/_http_lock은 직렬화를 유발하므로
# 워커 스레드(process_ai_object, process_data, get_llm_model)에서는 이 함수를 사용한다.
