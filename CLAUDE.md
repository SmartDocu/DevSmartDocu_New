...
- 사용자가 "후속 작업에 기록해줘"라고 요청하면(대상 파일을 별도로 지정하지 않는 한) `docs/aws-backend-deployment.md`의 "후속 작업 (미완료)" 섹션에 그 파일의 기존 항목과 같은 형식(`- [ ] **제목** — 설명, 날짜`)으로 기록한다. 메모리(auto memory)에는 남기지 않는다.
