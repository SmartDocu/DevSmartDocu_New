...
- [x] ~~옛 리소스(`smartdocu-backend-*`) 정리~~ — 2026-09-02 발견, 2026-09-08 완료. ALB(`smartdocu-backend-alb`)/ECS 서비스(`smartdocu-backend-service`)/타겟그룹(`smartdocu-backend-tg`) 3개는 AWS CLI로 삭제(각각 `LoadBalancerNotFound`/`INACTIVE`/`TargetGroupNotFound`로 확인). ECR 리포지토리(`smartdocu-backend`)는 `smartdocu-app` IAM 사용자에 `ecr:DeleteRepository` 권한이 없어(`AccessDeniedException`) 사용자가 콘솔에서 이미지 30개 삭제 후 리포지토리 자체도 직접 삭제 — `RepositoryNotFoundException`으로 완전 삭제 확인. 전 과정에서 현재 운영 중인 `d2doc-service`/`d2doc-alb`(별개 리소스, 별개 이름)는 영향 없음을 ECS 상태(ACTIVE, 1/1)·타겟그룹 헬스체크(healthy)·실제 `/health` HTTP 200으로 재확인 완료.
- [ ] **Supabase `sdoc` 스키마 RLS 적용** — 2026-09-02, `sdoc` 스키마 실제 테이블(삭제예정_/임시_/RPC 제외) 전수조사해서 [docs/rls-audit.html](rls-audit.html)로 정리함(적용 권장 47 / 결정 필요 62 / 적용 불필요 33, 뷰 4개 별도 — **2026-09-03 정정**: 최초 기록했던 "48/70/24"는 오기, 실제 파일 내용 기준 재계산값이 위 숫자다). 가장 시급한 건 `gendocs_realtimes`/`genchapters_realtimes`/`notifications`/`notification_users` 4개 — 프론트가 FastAPI를 거치지 않고 Supabase Realtime을 anon key + 사용자 JWT로 직접 구독하는 유일한 경로라, 이번 세션에 추가한 백엔드 소유권 검증이 전혀 안 걸림. `chat_sessions`/`chat_qas`/`chat_favorites`(d2chat)도 코드 주석에 RLS 적용을 전제로 설계됐다고 명시돼 있어 우선순위 높음. 착수 시점은 사용자가 별도로 지시.
  - **검증 완료(2026-09-03)**: 사용자가 `SUPABASE_DB_URL`(Supavisor 풀러)로 직접 접속을 시도했으나 두 풀러 모드(5432/6543) 모두 "tenant/user not found"로 실패, Direct connection은 IPv6 전용이라 이 환경에서 접속 불가 — 결국 사용자가 SQL 에디터에서 직접 `pg_tables` 조회해 결과를 공유하는 방식으로 진행함. **실제 RLS 상태 확인 결과: `sdoc` 스키마 174개 테이블(정리대상 36개 포함) 중 `app_translations` 단 1개만 RLS 켜짐, 나머지 173개 전부 꺼짐** — 감사표의 "결정 필요/적용 권장" 분류가 현재 전부 미적용 상태임을 실측으로 확인. 감사표와 실제 DB 테이블 목록을 전수 대조(diff)해 커버리지 100% 확인(감사표 142건 − 뷰 4개(`datas`/`ui_terms`/`ui_term_translations`/`vw_creditbucketsums`) = 실제 테이블 138개, DB 실제 174 − 정리대상 36 = 138로 정확히 일치).
  - **발견 및 수정한 버그(2026-09-03)**: `app_translations`가 유일하게 RLS ON 상태였는데 **정책(policy)이 하나도 없어서** anon/user-JWT 접근이 전부 차단되고 있었음(service-role 20건 조회 vs anon 0건 조회로 실측 확인). `backend/app/routers/apps.py`가 이 테이블을 오직 user-JWT(`_sb(token)`)로만 접근하도록 작성돼 있어 실제 영향 있었음: ① 앱 런처 화면에서 언어를 en/ja로 바꿔도 앱 이름 번역이 조용히 적용 안 됨, ② 관리자 앱 번역 관리 화면(`system.i18n.apps`)에서 기존 번역이 항상 빈 목록으로 보이고 저장 시도 시 "DB 저장 실패" 에러 발생 추정. 다른 전역 참조 테이블(`codes`/`menus`/`terms` 등)과 동일하게 `ALTER TABLE sdoc.app_translations DISABLE ROW LEVEL SECURITY;` 실행해 원상복구 — anon 권한으로 20건 전부 재조회되는 것 확인해 해결 완료.
  - **핵심 아이덴티티 그룹 RLS 적용 + 앱 레벨 취약점 수정 완료(2026-09-07)**: `chat_*`/`insight_*` 제외하고 "가장 시급한 4곳"(`gendocs_realtimes`/`genchapters_realtimes`/`notifications`/`notification_users`) 이후 핵심 아이덴티티 8개 테이블(`accounts`/`tenants`/`tenantusers`/`projects`/`projectusers`/`serviceusers`/`favorites`/`whitelists`) 전부 RLS 적용 + 정책 설계 + 더미 계정 교차접근 테스트까지 완료. `is_tenant_member(tenantid)`/`is_tenant_manager(tenantid)` SECURITY DEFINER 헬퍼 함수 2개를 만들어 재사용(self-row 트리거로 `tenantusers.rolecd`/`useyn`·`serviceusers.useyn`/`accountuid` 등 민감 필드의 자가변경도 차단).
    - **함께 발견·수정한 진짜 앱 레벨 취약점(전부 org.py 등의 "클라이언트가 보낸 tenantid/projectid/accountuid를 그대로 믿던" 동일 유형)**: `org.py`의 `tenant-users`/`projects`/`project-users` 엔드포인트 9곳(교차 테넌트 멤버 추가·삭제·조회, 프로젝트 소유권 미검증 수정 등), `accountuid` 쿼리파라미터를 통한 타 조직 서비스 정원/현재인원 정보노출 2곳, `whitelists.py`의 `save_whitelist()` IDOR(다른 테넌트 whitelistuid를 알면 가로채기 가능) 1곳 — 전부 수정 후 시뮬레이션 공격 테스트로 검증.
    - `settings.py`의 테넌트 매니저 self-edit 화면 3개 함수(`get_tenant_manage_tenant_info`/`get_tenant_manage_basic_info`/`save_tenant_manage_basic_info`)는 `tenants` 테이블 select/update만 service-role→user-JWT(`sb`)로 전환해 앱 체크+RLS 이중 방어 확보(그 외 아직 정책 없는 `accounts` 등은 service-role 유지).
    - **⚠️ RLS 적용 직후 실제 프로덕션 장애 2건 발생 → 즉시 발견·수정·재배포**: (1) 사용자(mm4707@naver.com) 실제 신고로 발견 — `accounts`에 RLS만 켜고 SELECT 정책을 안 만들어서, `get_service_permission()`(`require_doc_read`/`write` 등 Do/Ch/In 전체 서비스 권한판정의 핵심 함수, 8개 라우터에서 사용)이 user-JWT로 `accounts`를 조회할 때마다 전부 막혀 **모든 사용자의 문서/챕터/문장/차트/테이블 읽기·쓰기가 403("이 서비스에 대한 읽기 권한이 없습니다")으로 막히는 광범위 장애**가 발생함. `useruid=auth.uid() OR (useruid IS NULL AND is_tenant_member(tenantid))` SELECT 정책 추가로 즉시 해결(개인 계정은 본인만, 기업 공유 계정은 소속 멤버만 — `tenantid=1`을 전 개인계정이 공유해서 `is_tenant_member`만 걸면 남의 개인계정까지 노출되는 걸 `useruid IS NULL` 조건으로 차단). (2) 사용자 요청으로 `pg_tables`/`pg_policies` 집계 쿼리로 RLS 적용된 12개 테이블 전체를 SELECT/INSERT/UPDATE/DELETE 정책 유무까지 재점검하고 각 0개인 조합을 실제 코드와 전수 대조하다가 추가 발견 — `settings.py`의 시스템관리자(roleid=7) 전용 테넌트 관리 화면 3개(`list_tenants`/`save_tenant`/`delete_tenant`)가 `_require_admin()`으로 권한체크는 하면서 실제 DB 접근은 `sb`(사용자 JWT)를 쓰고 있어서, 관리자가 자기 소속과 무관한 임의의 테넌트를 다루는 이 화면 특성상 `tenants` RLS(소속 전제)와 근본적으로 안 맞아 신규 테넌트 생성 불가/소속없는 테넌트 수정 시 엉뚱하게 신규 생성으로 빠짐/삭제는 0행 삭제인데도 200 성공 응답하는 3중 결함이었음. 이 3곳은 `_require_admin()` 자체가 이미 충분한 권한 경계이므로 `svc`(service-role)로 되돌려 수정.
    - 두 장애 모두 더미 계정(개인+관리자)으로 실제 API 엔드투엔드 재현·검증(13개 체크 전부 통과) 후 Docker 빌드 → ECR push → `d2doc-service` force-new-deployment(rolloutState COMPLETED) → 프로덕션 `/health` 200 확인까지 완료. **남은 것**: `serviceusers`/`favorites`/`whitelists` 등은 이번에 이미 다 됨 — 이제 "결정 필요" 62개 테이블과 `chat_*`/`insight_*`(사용자가 명시적으로 나중으로 미룸)만 남음.
- [x] ~~Pro 구독 해지 3택(90일 유예삭제 / 즉시삭제 / Free 전환) — 개인+기업 테넌트 모두, Ch/In cascade 삭제, 회원 탈퇴까지 전부 구현+테스트+AWS 배포 완료~~ — 2026-09-03 구현 및 배포 완료.
  ...
  - **배포 완료(2026-09-03)**: 위 테넌트 해지 기능 + 사후 점검 3건 전부 포함해 Docker 빌드 → ECR push → `d2doc-service` force-new-deployment. 배포 도중 이전 태스크가 완전히 드레인되기 전에 `rolloutState`/이벤트 로그만 보고 완료로 오판했던 해프닝이 있었음(구 컨테이너를 테스트해 405 오탐 발생) — 이후 신구 태스크 running count로 재확인해 실제 완료 시점을 정확히 잡음. 최종적으로 `/health` 200, `POST /api/settings/tenant-manage/tenant-cancel`(무인증) 401(신규 라우트 정상 인식) 확인 완료. `d2doc-purge-daily-cron` EventBridge 스케줄러는 여전히 DISABLED 유지.
...
- [ ] **DISABLED 상태로 대기 중인 EventBridge Scheduler 2개 활성화(ENABLED) 필요** — 2026-09-14 기록. `d2doc-billing-daily-cron`(매일 00:00 KST, Lambda `d2doc-billing-cron` → `run-billing-cycle` 호출, 2026-08-27 생성)과 `d2doc-purge-daily-cron`(매일 00:30 KST, `content-purge/run-purge-cycle` 호출, 2026-09-03 생성) 둘 다 인프라·코드는 이미 완성돼 배포까지 끝났지만 실제 청구/파기 배치가 돌지 않도록 의도적으로 DISABLED로 유지 중. 착수 시 확인할 것: (1) 실제로 켜도 되는 시점인지(과금/삭제가 실제로 발생하므로 사용자 최종 확인 필수), (2) 켜기 전 `run-billing-cycle`/`run-purge-cycle` 대상 계정이 현재 몇 건인지 드라이런 확인 권장, (3) 상세 리소스명/켜는 명령어는 메모리 `portone-integration-progress`와 이 문서 82행·108행 참고. **절대 사용자의 명시적 요청 없이 먼저 켜지 말 것.**
- [ ] **팝업 "노출 시점"(`popups.mainlogin`) 값별 분기 로직 구현** — 2026-09-14 admin/popups 화면 작업 중 발견. `codes`(`codegroupcd='popup_mainlogin'`)에 `M`="Home Page", `L`="After Login (Not Implemented)" 두 값이 등록돼 있고 관리자 화면(`AdminPopupsPage.jsx`)에서 선택은 가능하지만, 실제 팝업을 렌더링하는 `frontend/src/components/Popup/PopupManager.jsx`는 `mainlogin` 필드를 아예 참조하지 않음 — 현재는 두 값 중 뭘 골라도 동작 차이 없이 무조건 노출됨(서버가 내려준 목록을 기간/활성 필터만 거쳐 그대로 표시). 착수 시 필요한 작업: (1) `L`(로그인 후) 선택 시 비로그인 사용자에게는 숨기는 조건을 `PopupManager.jsx`의 `visible` 필터에 추가, (2) 필요하면 `M`/`L` 노출 위치 자체도 구분할지(현재는 `PopupManager`가 어디 마운트되느냐로만 결정됨, HomePage 등) 확인.
- [x] ~~삭제 버튼 확인창(Modal.confirm) 누락/불일치 정리~~ — 2026-09-14 전체 프론트엔드 감사(delete 액션 39개 파일) 결과 발견된 8개 파일 전부 수정 완료. 상세:
  - **확인창 자체가 없어서 바로 삭제되던 곳(4개 파일, 9곳)** — `modal.confirm`(`App.useApp()`) 추가 완료:
    - `frontend/src/pages/d2chat/D2ChatPage.jsx` — `handleSidebarDelete`를 `performSidebarDelete(id, section)`+확인 래퍼로 분리(즐겨찾기/보낸공유/받은공유/히스토리 4가지 케이스 공통 적용).
    - `frontend/src/pages/d2insight/D2InsightPage.jsx` — `handleSidebarDelete`(동일 패턴으로 분리), `handleUnshareSchedule`, `handleDeleteReceivedScheduleShare` 3곳 모두 `modal.confirm` 추가.
    - `frontend/src/pages/public/QnaPage.jsx` — "답변 삭제"(`handleAnswerDelete`)에 `modal.confirm` 추가. 같은 파일의 QNA 행 삭제(`handleDelete`)도 구식 `Popconfirm`이었어서 함께 `modal.confirm`으로 교체(아래 항목과 중복 처리).
    - `frontend/src/pages/NotificationsPage.jsx` — `handleDelete`에 `modal.confirm` 추가.
  - **구식 `Popconfirm` → `Modal.confirm`으로 교체 완료** — `frontend/src/pages/settings/SettingsLlmKeysPage.jsx`, `frontend/src/pages/org/OrgProjectUsersPage.jsx`, `frontend/src/pages/public/QnaPage.jsx`(QNA 행 삭제). 3개 파일 모두 `Popconfirm` import 제거.
  - **`Popconfirm`+`Modal.confirm` 중첩(이중 확인) 정리 완료** — `frontend/src/pages/org/OrgTenantUsersPage.jsx`, `frontend/src/pages/org/OrgProjectsPage.jsx`. 기존 `handleDelete` 내부의 `Modal.confirm`은 이미 정상이라 그대로 두고, 버튼을 감싸던 `Popconfirm` 래퍼만 제거(버튼 `onClick={handleDelete}`로 직결).
  - 나머지 35개 파일은 이미 `Modal.confirm`/`modal.confirm` 표준을 따르고 있어 손대지 않음. `MyInfoPage.jsx`의 마케팅 수신동의 토글용 `Popconfirm`(218행)은 삭제 액션이 아니라 감사 대상이 아니었으므로 이번 작업에서 제외(그대로 유지).
  - 빌드 전수 통과 확인(2026-09-14).
