# 다국어 Term Key 목록

프로젝트 전체 수집 기준일: 2026-05-28 (최종 업데이트)  
ui_terms 테이블 기준 (DB 조회 결과로 업데이트)

---

## 1. cod. (36개)

### code.align (3개)

| term_key |
|----------|
| cod.align_center |
| cod.align_left |
| cod.align_right |

### code.databasiscd (3개)

| term_key |
|----------|
| cod.databasiscd_dbq |
| cod.databasiscd_dbs |
| cod.databasiscd_dbt |

### code.datasourcecd (2개)

| term_key |
|----------|
| cod.datasourcecd_df |
| cod.datasourcecd_dfv |

### code.is_multirow (2개)

| term_key |
|----------|
| cod.is_multirow_n |
| cod.is_multirow_y |

### code.keycoldatatypecd (8개)

| term_key |
|----------|
| cod.keycoldatatypecd_boolean |
| cod.keycoldatatypecd_currency |
| cod.keycoldatatypecd_date |
| cod.keycoldatatypecd_datetime |
| cod.keycoldatatypecd_identifier |
| cod.keycoldatatypecd_number |
| cod.keycoldatatypecd_string |
| cod.keycoldatatypecd_text |

### code.llmmodeluseyn (2개)

| term_key |
|----------|
| cod.llmmodeluseyn_C |
| cod.llmmodeluseyn_S |

### code.useyn (2개)

| term_key |
|----------|
| cod.useyn_n |
| cod.useyn_y |

### code.common (6개)

| term_key |
|----------|
| cod.filter_all |
| cod.status_active |
| cod.status_inactive |
| cod.sep_newusers |
| cod.rolecd_M |
| cod.rolecd_U |

### code.billing (4개)

| term_key |
|----------|
| cod.billing_Fr |
| cod.billing_Pr |
| cod.billing_Te |
| cod.billing_En |

### code.qna (4개)

| term_key |
|----------|
| cod.qna_answered |
| cod.approve_pending |
| cod.approve_rejected |
| cod.qna_pending |

---

## 2. msg. (184개)

| term_key |
|----------|
| msg.col.mapping.required |
| msg.chapter.unselected |
| msg.confirm.object.apply |
| msg.loading.doc.merging |
| msg.loading.object.applying |
| msg.loading.object.writing |
| msg.loading.preparing |
| msg.object.apply.complete |
| msg.object.empty |
| msg.object.write.complete |
| msg.doc.already.writing |
| msg.loading.chapter.count |
| msg.doc.param.duplicate |
| msg.doc.write.complete |
| msg.docid.load.error |
| msg.loading.chapter.finalizing |
| msg.loading.chapter.preparing |
| msg.loading.chapter.progress |
| msg.loading.doc.writing |
| msg.loading.gendoc |
| msg.loading.gendoc.delete |
| msg.loading.params |
| msg.param.required |
| msg.ph.gendocnm |
| msg.ph.search |
| msg.unset.objects |
| msg.ai.input.required |
| msg.api.param.required |
| msg.authtest.required |
| msg.baseurl.required |
| msg.chart.select.required |
| msg.chapter.required |
| msg.chapter.select.delete |
| msg.chat.tables.required |
| msg.code.groupcd.required |
| msg.code.select.delete |
| msg.code.select.trans |
| msg.code.value.required |
| msg.col.empty |
| msg.coming.soon |
| msg.confirm.delete |
| msg.confirm.doc.close |
| msg.confirm.doc.open |
| msg.loading.doc.close |
| msg.loading.doc.open |
| msg.contact.required |
| msg.contact.success |
| msg.confirm.file.overwrite |
| msg.confirm.objecttype.change |
| msg.connectnm.required |
| msg.connnm.required |
| msg.datanm.required |
| msg.dataset.filter.readonly |
| msg.db.required |
| msg.delete.error |
| msg.delete.success |
| msg.deleted |
| msg.doc.empty |
| msg.doc.loading |
| msg.doc.required |
| msg.doc.select |
| msg.doc.select.delete |
| msg.docselect.error |
| msg.docselect.required |
| msg.docselect.tabs.close |
| msg.email.required |
| msg.favorite.error |
| msg.file.required |
| msg.gendoc.closed |
| msg.gendoc.opened |
| msg.init.load.error |
| msg.invalid.objecttype |
| msg.load.error |
| msg.llmmodel.required |
| msg.llmmodelnm.required |
| msg.loading |
| msg.usetypecd.required |
| msg.loading.wait |
| msg.login.failed |
| msg.login.success |
| msg.menu.required |
| msg.menu.select.delete |
| msg.menu.select.trans |
| msg.message.required |
| msg.message.select.delete |
| msg.message.select.trans |
| msg.no.change.history |
| msg.no.chapter |
| msg.no.filter.info |
| msg.no.image |
| msg.no.data |
| msg.no.data.to.save |
| msg.no.file.selected |
| msg.no.edit.permission |
| msg.no.object.type |
| msg.notification.coming.soon |
| msg.objectnm.required |
| msg.password.minlength |
| msg.password.mismatch |
| msg.password.required |
| msg.ph.email |
| msg.ph.prompt |
| msg.placeholder.password.change |
| msg.placeholder.password.confirm |
| msg.placeholder.rel.path |
| msg.placeholder.secret.change |
| msg.preparing |
| msg.preview.error |
| msg.projectnm.required |
| msg.prompt.key.required |
| msg.prompt.preview.hint |
| msg.prompt.prompttypecd.required |
| msg.prompt.required |
| msg.prompt.select.delete |
| msg.prompt.select.trans |
| msg.prompt.text1.required |
| msg.reject.reason.required |
| msg.register.failed |
| msg.register.success |
| msg.register.tenant.required |
| msg.register.terms.required |
| msg.required |
| msg.reset.failed |
| msg.reset.sent |
| msg.sample.empty |
| msg.save.col.created |
| msg.save.col.warn |
| msg.save.error |
| msg.save.success |
| msg.saved.name |
| msg.saved |
| msg.select |
| msg.select.chapter |
| msg.select.connector |
| msg.select.data |
| msg.select.object |
| msg.select.server |
| msg.select.delete |
| msg.select.placeholder |
| msg.select.project |
| msg.sentence.preview.required |
| msg.server.error |
| msg.sidebar.favorites |
| msg.sidebar.search |
| msg.source.select |
| msg.tab.maxcount |
| msg.tenant.user.reject.reason |
| msg.tenantnm.required |
| msg.template.none |
| msg.term.required |
| msg.term.select.delete |
| msg.term.select.trans |
| msg.unknown.error |
| msg.usernm.required |
| msg.qna.private |
| msg.qna.title.required |
| msg.qna.question.required |
| msg.url.required |
| msg.dataset.required |
| msg.editor.not.ready |
| msg.filter.map.saved |
| msg.filter.save.error |
| msg.objects.extracted |
| msg.scaparams.empty |
| msg.tblparams.empty |
| msg.tmpl.click.extract |
| msg.tmpl.err.else.no.if |
| msg.tmpl.err.else.typo |
| msg.tmpl.err.endfor.no.for |
| msg.tmpl.err.endif.no.if |
| msg.tmpl.err.for.typo |
| msg.tmpl.err.if.typo |
| msg.tmpl.err.unclosed |
| msg.tmpl.input.hint |
| msg.ph.dataset |
| msg.ph.column |
| msg.password.changed |
| msg.password.change.failed |
| msg.password.reset.invalid |
| msg.new.password.required |
| msg.password.min6 |
| msg.password.confirm.required |
| msg.ph.sentence.template |
| msg.no.docid |

---

## 3. btn. (66개)

| term_key |
|----------|
| btn.1year |
| btn.3months |
| btn.add |
| btn.answer_btn |
| btn.api.setting |
| btn.apply |
| btn.back |
| btn.back.to.login |
| btn.cancel |
| btn.chapter.read |
| btn.chapter.rewrite |
| btn.close |
| btn.color.hide |
| btn.color.show |
| btn.colormap.hide |
| btn.colormap.show |
| btn.confirm |
| btn.delete |
| btn.doc.close |
| btn.doc.open |
| btn.doc.read |
| btn.doc.status |
| btn.doc.write |
| btn.doc.write.all |
| btn.doc.write.combine |
| btn.download.chapter |
| btn.download.doc |
| btn.download.modified |
| btn.download.modified.chapter |
| btn.extract.objects |
| btn.go.login |
| btn.insert.to.editor |
| btn.item.manage |
| btn.login.ing |
| btn.login_btn |
| btn.logout |
| btn.lookup |
| btn.new |
| btn.object.apply |
| btn.object.manage |
| btn.object.rewrite |
| btn.objectconfig |
| btn.ok |
| btn.password.change |
| btn.preview_btn |
| btn.prompt.replace |
| btn.register.ing |
| btn.register_btn |
| btn.reset.close |
| btn.reset.password |
| btn.reset.send |
| btn.sample.prompt |
| btn.save |
| btn.savecols |
| btn.send |
| btn.setting |
| btn.tab.close.left |
| btn.tab.close.others |
| btn.tab.close.right |
| btn.tab.close.this |
| btn.template.edit |
| btn.test |
| btn.upgrade |
| btn.upload.chapter |
| btn.upload.modified |
| btn.upload_btn |

---

## 4. lbl. (197개)

### 일반 레이블 (195개)

| term_key |
|----------|
| lbl.address |
| lbl.agree.all |
| lbl.agreed |
| lbl.aliases |
| lbl.align |
| lbl.answer |
| lbl.answerdts |
| lbl.answernm |
| lbl.api_key_lbl |
| lbl.api_key_locationcd_lbl |
| lbl.api_key_name_lbl |
| lbl.api_key_value_lbl |
| lbl.apikey |
| lbl.authored.chapter |
| lbl.authored.doc |
| lbl.authorization_type_lbl |
| lbl.authtest_lbl |
| lbl.authtype_lbl |
| lbl.baseurl_lbl |
| lbl.bgcolor |
| lbl.billing.model |
| lbl.billing.multi |
| lbl.billing.single |
| lbl.billingusercnt |
| lbl.bold |
| lbl.bordercolor |
| lbl.chapter |
| lbl.chapter.create.dts |
| lbl.chapter.no |
| lbl.chapternm |
| lbl.chapterno |
| lbl.chapters |
| lbl.child_column |
| lbl.ckeditor.body_text |
| lbl.ckeditor.heading1 |
| lbl.ckeditor.heading2 |
| lbl.ckeditor.heading3 |
| lbl.ckeditor.heading4 |
| lbl.codegroupcd_lbl |
| lbl.codevalue_lbl |
| lbl.col.select |
| lbl.colnames |
| lbl.color.ref |
| lbl.connectnm_lbl |
| lbl.connnm_lbl |
| lbl.create.period |
| lbl.createdts |
| lbl.creatornm |
| lbl.databasiscd |
| lbl.datanm_lbl |
| lbl.dataset.unused |
| lbl.dataset.used |
| lbl.dataset_lbl |
| lbl.datasourcecd_lbl |
| lbl.datauid |
| lbl.datauid_lbl |
| lbl.db_lbl |
| lbl.dbtype_lbl |
| lbl.default_message |
| lbl.default_message_lbl |
| lbl.default_name_lbl |
| lbl.default_text_lbl |
| lbl.default_time_column |
| lbl.desc_lbl |
| lbl.dim.cols |
| lbl.doc |
| lbl.doc.create.dts |
| lbl.doc.createfiledts |
| lbl.doc.final.dts |
| lbl.doc.upload.dts |
| lbl.docnm |
| lbl.done |
| lbl.email |
| lbl.endpoint_lbl |
| lbl.file |
| lbl.filter.col.map |
| lbl.filter.dataset |
| lbl.filter.var |
| lbl.fontcolor |
| lbl.fontsize |
| lbl.grain |
| lbl.grant_type_lbl |
| lbl.health_lbl |
| lbl.iconnm_lbl |
| lbl.is_active_lbl |
| lbl.is_default_lbl |
| lbl.isdefault |
| lbl.isprivate |
| lbl.issystemtenant |
| lbl.items |
| lbl.jobnm |
| lbl.joindt |
| lbl.keycoldatatypecd |
| lbl.keycolnm |
| lbl.llmlimityn |
| lbl.llmmodelnicknm |
| lbl.llmmodelnm |
| lbl.llmvendornm |
| lbl.measure.cols |
| lbl.menucd_lbl |
| lbl.message_lbl |
| lbl.messagekey_lbl |
| lbl.messagetypecd_lbl |
| lbl.multi.var |
| lbl.myrole |
| lbl.name |
| lbl.new.password |
| lbl.newuser |
| lbl.nmcolnm |
| lbl.not.agreed |
| lbl.note |
| lbl.oauth_client_id_lbl |
| lbl.oauth_client_secret_lbl |
| lbl.objectdesc_lbl |
| lbl.objectnm_lbl |
| lbl.objecttypecd_lbl |
| lbl.operator_lbl |
| lbl.optional |
| lbl.ordercolnm |
| lbl.orderno_lbl |
| lbl.paramnm_lbl |
| lbl.parent_column |
| lbl.parent_schema |
| lbl.parent_table |
| lbl.password |
| lbl.password.confirm |
| lbl.password_lbl |
| lbl.plan |
| lbl.plan.free |
| lbl.plan.pro |
| lbl.popup.days_hide_suffix |
| lbl.port_lbl |
| lbl.primary_key |
| lbl.project |
| lbl.projectnm_lbl |
| lbl.prompt |
| lbl.promptkey_lbl |
| lbl.prompttypecd_lbl |
| lbl.purpose |
| lbl.query_examples |
| lbl.question |
| lbl.redirect_url_lbl |
| lbl.refresh_token_lbl |
| lbl.required |
| lbl.reset.email |
| lbl.retry_count_lbl |
| lbl.rolecd_lbl |
| lbl.route_path |
| lbl.sample |
| lbl.samplevalue |
| lbl.sca.params |
| lbl.sca.params.list |
| lbl.scope_lbl |
| lbl.select |
| lbl.sentence.type |
| lbl.server_lbl |
| lbl.service_name_lbl |
| lbl.sid_lbl |
| lbl.single.var |
| lbl.sort |
| lbl.source.data |
| lbl.ssl_mode_lbl |
| lbl.status |
| lbl.step |
| lbl.subject |
| lbl.supabase_url_lbl |
| lbl.syntax.select |
| lbl.tag1_lbl |
| lbl.tag2_lbl |
| lbl.tblparams.list |
| lbl.telno |
| lbl.template.upload |
| lbl.tenant |
| lbl.tenant.icon |
| lbl.tenantnm |
| lbl.termgroupcd_lbl |
| lbl.termkey_lbl |
| lbl.terms.agree |
| lbl.terms.marketing |
| lbl.terms.privacy |
| lbl.terms.service |
| lbl.theme |
| lbl.timeout_lbl |
| lbl.timezone |
| lbl.tns_lbl |
| lbl.token_endpoint_lbl |
| lbl.total.chapters |
| lbl.unreflected.chapters |
| lbl.unreflected.objects |
| lbl.upload_lbl |
| lbl.uploaded.chapter |
| lbl.uploaded.doc |
| lbl.username_lbl |
| lbl.usernm |
| lbl.useyn_lbl |

### lbl.chart 기타 (2개)

| term_key |
|----------|
| lbl.chart.size |
| lbl.chart.type_lbl |

---

## 5. thd. (97개)

| term_key |
|----------|
| thd.activeyn_thd |
| thd.align_thd |
| thd.authtype_thd |
| thd.auto.write |
| thd.bgcolor_thd |
| thd.billingmodelcd |
| thd.bold_thd |
| thd.chapternm |
| thd.closeuser |
| thd.closeyn |
| thd.codegroupcd_thd |
| thd.codevalue_thd |
| thd.colnm |
| thd.comma |
| thd.configured_thd |
| thd.connectnm_thd |
| thd.connnm_thd |
| thd.createfiledts |
| thd.createuser |
| thd.datanm_thd |
| thd.dataset.col |
| thd.datasourcecd_thd |
| thd.datatypecd |
| thd.dbtype_thd |
| thd.decimal |
| thd.default_name_thd |
| thd.default_text_thd |
| thd.desc_thd |
| thd.dispcolnm |
| thd.docparam_thd |
| thd.email_thd |
| thd.field_path_thd |
| thd.filter.col |
| thd.filterjson |
| thd.fixed_value_thd |
| thd.fontcolor_thd |
| thd.fontsize_thd |
| thd.gendocnm |
| thd.inputvalue |
| thd.is_fixed_thd |
| thd.is_required_thd |
| thd.languagecd |
| thd.languagenm |
| thd.llmmodelnicknm_thd |
| thd.llmmodelnm_thd |
| thd.llmmodeluseyn_thd |
| thd.llmvendornm_thd |
| thd.logical_name_thd |
| thd.measureyn |
| thd.menucd_thd |
| thd.messagekey_thd |
| thd.messagetypecd_thd |
| thd.modified.upload |
| thd.move |
| thd.new.chapter |
| thd.new.genobject.yn |
| thd.new.object.yn |
| thd.new.upload |
| thd.new_chapteryn |
| thd.new_object_cnt |
| thd.obj.setting.dts |
| thd.obj.write.dts |
| thd.object_cnt |
| thd.objectdesc_thd |
| thd.objectnm_thd |
| thd.objectsettingyn |
| thd.objecttypecd_thd |
| thd.operator_thd |
| thd.orderno_thd |
| thd.param_location_thd |
| thd.paramnm_thd |
| thd.params |
| thd.projectdesc_thd |
| thd.projectnm_thd |
| thd.promptkey_thd |
| thd.prompttypecd_thd |
| thd.querycolnm |
| thd.rolecd_thd |
| thd.setting_thd |
| thd.tag1_thd |
| thd.tenantnm_thd |
| thd.termgroupcd_thd |
| thd.termkey_thd |
| thd.testvalue_thd |
| thd.translated_desc |
| thd.translated_text |
| thd.translated_text1_thd |
| thd.translated_text2_thd |
| thd.translated_title_thd |
| thd.updatefiledts |
| thd.updateuser |
| thd.usernm_thd |
| thd.usetypecd_thd |
| thd.useyn_thd |
| thd.value_count_thd |
| thd.value_thd |
| thd.width_px |

---

## 6. ttl. (60개)

| term_key |
|----------|
| ttl.ai.chart.manage |
| ttl.ai.sentence.manage |
| ttl.ai.table.manage |
| ttl.api.param.setting |
| ttl.api.params |
| ttl.chapter.list |
| ttl.chapter.objects |
| ttl.chapter.status |
| ttl.chart.manage |
| ttl.chart.settings |
| ttl.chart.type |
| ttl.col.info |
| ttl.condition |
| ttl.confirm.delete |
| ttl.data.list |
| ttl.data.preview |
| ttl.datacols |
| ttl.dataset.mapping |
| ttl.dataset_ttl |
| ttl.detail |
| ttl.doc.config |
| ttl.doc.list |
| ttl.doc.read_ttl |
| ttl.doc.select |
| ttl.doc.status_ttl |
| ttl.doc.write_ttl |
| ttl.docselect |
| ttl.docselect.change |
| ttl.filter.map.setting |
| ttl.header.settings |
| ttl.help |
| ttl.list |
| ttl.llm.detail |
| ttl.login_ttl |
| ttl.myinfo.personal |
| ttl.myinfo.projects |
| ttl.myinfo.tenant |
| ttl.myinfo.tenant.history |
| ttl.myinfo.terms |
| ttl.object.detail |
| ttl.param.input |
| ttl.password.reset |
| ttl.preview.result |
| ttl.preview_ttl |
| ttl.project.llm |
| ttl.prompt.desc |
| ttl.prompt_ttl |
| ttl.qna.answer |
| ttl.register_ttl |
| ttl.sample.list |
| ttl.sample.prompt_ttl |
| ttl.sentence.manage |
| ttl.table.manage |
| ttl.tenant.llm |
| ttl.tenant.user.reject |
| ttl.tmpl.syntax.error |
| ttl.translations |
| ttl.user.select |
| ttl.value.select |
| ttl.value.settings |

---

## 7. inf. (10개)

| term_key |
|----------|
| inf.doc.preview.notice |
| inf.gensentence.default |
| inf.iconnm_inf |
| inf.password.hidden |
| inf.preview.empty |
| inf.preview.rows |
| inf.save.before.navigate |
| inf.secret.hidden |
| inf.tmpl.default |
| inf.tmpl.insert |

---

## 요약

| prefix | 개수 |
|--------|------|
| cod.   | 36   |
| msg.   | 184  |
| btn.   | 66   |
| lbl.   | 197  |
| thd.   | 97   |
| ttl.   | 60   |
| inf.   | 10   |
| **합계** | **650** |


---

## 번역 코드 미적용 대상 제외 목록

### ⚠️ t() 미적용 대상 제외 — 한국어/영어 버전 분리 방식으로 처리

| 파일 | 비고 |
|------|------|
| `frontend/src/pages/HomePage.jsx` | 서비스 소개 랜딩 페이지 |
| `frontend/src/pages/public/ServicePage.jsx` | 서비스 안내 페이지 |
| `frontend/src/pages/public/ExperiencePage.jsx` | AI 체험하기 페이지 |
| `frontend/src/pages/public/FollowPage.jsx` | 따라하기 페이지 |
| `frontend/src/pages/public/AboutPage.jsx` | 기능 소개 페이지 |
| `frontend/src/pages/public/TermsPage.jsx` | 약관 페이지 |
| `frontend/src/pages/public/UsagePage.jsx` | 서비스 이용 요금 페이지 |
| `frontend/src/components/TenantRequestModal/TenantRequestModal.jsx` | 기업 등록/등록 요청 모달 |

### ⚠️ t() 미적용 대상 제외 — System Admin 대상 페이지

| 파일 | 비고 |
|------|------|
| `frontend/src/pages/admin/AdminHelpsPage.jsx` | 시스템 관리자 전용, 다국어 불필요 |
| `frontend/src/pages/admin/AdminTenantRequestsPage.jsx` | 기업 등록 요청 관리 페이지 |
