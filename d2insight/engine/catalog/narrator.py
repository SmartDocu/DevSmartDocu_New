"""섹션 해설자 — 섹션의 표·수치를 근거로 본문 해설을 쓴다 (§7.1, 결정 2026-07-14).

호출 단위는 **섹션 1회**다. 같은 섹션의 모듈들은 하나의 이야기를 이루므로, 해설자가 그 섹션의
...
섹션 하나에 속한 모듈들의 표와 수치를 받아, 모듈마다 해설 문단을 쓴다.
...
5. 같은 섹션의 다른 모듈 결과와 이어지게 쓴다(앞 모듈이 짚은 것을 뒤 모듈이 받아 설명).
...
def _pick_grade(items: list[dict]) -> str:
    """섹션에 섞인 모듈 등급 중 가장 높은 등급으로 1회 호출한다."""
