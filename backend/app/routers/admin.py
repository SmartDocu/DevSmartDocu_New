...
def list_llmapis(token: str = Depends(get_token)):
    ...
    tenants = sb.schema(SUPABASE_SCHEMA).table("tenants").select("tenantnm").eq("useyn", True).or_("issystemtenant.is.null,issystemtenant.eq.false").order("tenantid").execute().data or []
